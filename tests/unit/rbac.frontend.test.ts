...
describe('RBAC visual por rol (frontend)', () => {
  it('Super Admin ve todos los módulos operativos (30)', () => {
  ...
  it('Soporte: dashboard/noc/crm/commercial/support/tech-pwa; no billing ni mikrotik', () => {
    expect(getAllowedTabsByRole('Soporte')).toEqual(['dashboard', 'noc', 'crm', 'commercial', 'support', 'tech-pwa', 'inventory-routers', 'gis', 'portal', 'manual-safe-mode', 'safe-command-queue', 'routeros-readonly', 'inventory-sync', 'provisioning', 'user-manual']);
...
describe('Visibilidad en sidebar ≠ acceso (módulos internos ocultos)', () => {
  ...
  it('isSidebarHiddenTab marca exactamente los 11 módulos ocultos del menú WISP', () => {
  ...
  it('Super Admin: módulos internos accesibles pero ocultos del sidebar', () => {
    const retired = new Set(['automation', 'notifications']);
    const accessibleHidden = HIDDEN.filter((id) => !retired.has(id));
    ...
    // automation/notifications: retirados de la UI WISP (sin acceso por rol)
    expect(canAccessTab('Super Admin', 'automation')).toBe(false);
    expect(canAccessTab('Super Admin', 'notifications')).toBe(false);
    expect(isVisibleInSidebar('Super Admin', 'automation')).toBe(false);
    expect(isVisibleInSidebar('Super Admin', 'notifications')).toBe(false);
  });
