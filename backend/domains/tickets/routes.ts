...
router.get('/api/technicians', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  res.json(await svc().listTechnicians(tenantIdFromRequest(req)));
}));

router.get('/api/tickets/sla/breaches', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  res.json({ rules: getSlaRules(), breaches: await listSlaBreachesFromSupport(tenantIdFromRequest(req)) });
}));

router.get('/api/tickets', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  res.json(await svc().listTickets(req.query as Record<string, unknown>, tenantIdFromRequest(req)));
}));

router.get('/api/tickets/:id', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  const ticket = await svc().getTicket(req.params.id, tenantIdFromRequest(req));
...
router.post('/api/tickets', requireRoles([...WRITE_TICKET_ROLES]), asyncHandler(async (req, res) => {
  const created = await svc().createTicket(req.body || {}, tenantIdFromRequest(req));
  res.status(201).json(created);
}));

router.put('/api/tickets/:id', requireRoles([...WRITE_TICKET_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().updateTicket(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));

router.delete('/api/tickets/:id', requireRoles(['super admin', 'administrador']), asyncHandler(async (req, res) => {
  const ok = await svc().deleteTicket(req.params.id, tenantIdFromRequest(req));
...
router.post('/api/tickets/:id/assign', requireRoles([...WRITE_TICKET_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().assignTicket(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));

router.post('/api/tickets/:id/status', requireRoles([...TECH_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().setTicketStatus(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));

router.post('/api/tickets/:id/message', requireRoles([...TECH_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().addTicketMessage(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));

router.post('/api/tickets/:id/attachments', requireRoles([...TECH_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().addTicketAttachment(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));

router.get('/api/tickets/:id/history', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  const tenantId = tenantIdFromRequest(req);
  const ticket = await svc().getTicket(req.params.id, tenantId);
  if (!ticket) throw new NotFoundError('Ticket not found', 'NOT_FOUND');
  res.json(await svc().getTicketHistory(req.params.id, tenantId));
}));

router.get('/api/workorders/agenda', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  res.json(await svc().getWorkOrderAgenda(req.query as Record<string, unknown>, tenantIdFromRequest(req)));
}));

router.get('/api/workorders', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  res.json(await svc().listWorkOrders(req.query as Record<string, unknown>, tenantIdFromRequest(req)));
}));

router.get('/api/workorders/:id', requireRoles(READ_ROLES), asyncHandler(async (req, res) => {
  const order = await svc().getWorkOrder(req.params.id, tenantIdFromRequest(req));
...
router.post('/api/workorders', requireRoles([...WRITE_WO_ROLES]), asyncHandler(async (req, res) => {
  const created = await svc().createWorkOrder(req.body || {}, tenantIdFromRequest(req));
  res.status(201).json(created);
}));

router.put('/api/workorders/:id', requireRoles([...WRITE_WO_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().updateWorkOrder(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));

router.delete('/api/workorders/:id', requireRoles(['super admin', 'administrador']), asyncHandler(async (req, res) => {
  const ok = await svc().deleteWorkOrder(req.params.id, tenantIdFromRequest(req));
...
router.post('/api/workorders/:id/checklist/:index/toggle', requireRoles([...TECH_ROLES]), asyncHandler(async (req, res) => {
  ...
  res.json(await svc().toggleChecklistItem(req.params.id, index, tenantIdFromRequest(req)));
}));

router.post('/api/workorders/:id/update-status', requireRoles([...TECH_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().updateWorkOrderStatus(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));
...
router.post('/api/workorders/:id/evidences', requireRoles([...TECH_ROLES]), asyncHandler(async (req, res) => {
  res.json(await svc().addWorkOrderEvidence(req.params.id, req.body || {}, tenantIdFromRequest(req)));
}));
  ...
  for (const item of items) {
    try {
      if (item.action === 'status') {
        await svc().updateWorkOrderStatus(item.orderId, item.payload || {}, tenantId);
      } else if (item.action === 'evidence') {
        await svc().addWorkOrderEvidence(item.orderId, item.payload || {}, tenantId);
      } else if (item.action === 'checklist') {
        await svc().toggleChecklistItem(item.orderId, Number(item.payload?.index ?? 0), tenantId);
      }
