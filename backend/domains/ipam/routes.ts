...
router.get('/api/ipam/routers', requireRoles(READ_ROLES), (_req, res) => {
  res.json(ipamService.listRouters());
});

router.get('/api/ipam/routers/:routerId/pools', requireRoles(READ_ROLES), (req, res) => {
  const pools = ipamService.listPools(req.params.routerId);
  if (!pools) {
