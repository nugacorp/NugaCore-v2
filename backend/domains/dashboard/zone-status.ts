...
// READ-ONLY: combina Network (sitios) + NOC (routers) + SNMP live.
// ====================================================================
...
export interface ZoneEquipmentRow {
  ...
  brand: string;
  detail?: string;
  /** snmp-live = poll SNMP reciente; router-telemetry = API NOC; site = estado del sitio */
  source: 'snmp-live' | 'router-telemetry' | 'site';
}
...
const resolveRouterStatus = (
  ...
  if (isSnmpResultFresh(snmp)) {
    return {
      ...
      source: 'snmp-live',
      detail: snmp?.sysName
        ? `SNMP ${snmp.sysName} · ${snmp.latencyMs ?? 0}ms`
        : `SNMP up · ${snmp?.latencyMs ?? 0}ms`,
    };
  }

  if (router.isOnline) {
    return {
      status: healthToEquipment(resolveHealthStatus(router, referenceTimestampMs)),
      source: 'router-telemetry',
      detail: `CPU ${router.cpuUsagePct}% · RAM ${router.memoryUsagePct}%`,
    };
  }

  return { status: 'offline', source: 'router-telemetry', detail: 'Sin conexión API' };
};
...
export const buildZoneEquipment = (
  ...
  const rows: ZoneEquipmentRow[] = tower.equipment.map((item, index) => {
    ...
    if (isRouterEquipment(role)) {
      ...
      if (matched) {
        ...
        const resolved = resolveRouterStatus(matched, referenceTimestampMs);
  ...
  for (const router of linkedRouters) {
    ...
    const resolved = resolveRouterStatus(router, referenceTimestampMs);
    rows.push({
      id: router.id,
      name: router.name,
      role: 'router',
