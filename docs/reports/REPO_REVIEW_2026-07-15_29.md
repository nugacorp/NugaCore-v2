...
Periodo cubierto: **15 al 29 de julio de 2026** (el merge del último PR quedó
registrado el 30 a las 05:02 UTC).
...
| --- | --- |
| Commits en `main` | **136** |
| PRs mergeados | **66** (`#15` → `#82`) |
| PRs abiertos | **2** (`#80`, `#81`) |
| Archivos tocados | 299 |
| Líneas | +24 795 / −2 455 |
| Migraciones nuevas | 23 |
...
- Documentación: `architecture/MULTI_TENANT.md` (7 revisiones en el periodo).

### 2. Auth, registro público y onboarding WISP (16–17 jul)

Una cadena de 12 PRs sobre el alta real de WISPs, casi todos correcciones
encontradas en uso:
...
`#22`/`#23` portal del cliente y App Técnicos usables con enlace para compartir ·
`#65` administrar portal del cliente y sección Apps · `#56` modal de perfil sobre
el sidebar · `#24` eliminar clientes de prueba que inflaban el MRR · `#25`
recuperar chunks Vite obsoletos tras deploy · `#27` Routers en Sistema y limpieza
de inventarios huérfanos · finanzas sin datos mock, DELETE de gastos y alta
...
Las 20 migraciones restantes del periodo sí están aplicadas y registradas.
...
`#81` incluye los dos commits (`6eed45f` de factibilidad y `c6ad482` del worker
OLT), por lo que se solapa con `#80`. Conviene decidir si `#81` va después de
`#80` o si se rebasa para contener solo su slice.
...
27 archivos y +2 575 líneas sin lint, typecheck, tests ni build ejecutados.
Además solapa los commits de `#80`.
...
6. **Ejecutar CI en `#81`** y decidir su relación con `#80` antes de revisarlo.
