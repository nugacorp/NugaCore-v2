...
- **El contrato de API v1.** Las rutas y formas de respuesta que `App.tsx` consume hoy (ver [API_ANALYSIS.md](../architecture/API_ANALYSIS.md) §6) son intocables salvo cambio aditivo o adaptación del frontend en el mismo PR.
- **Los tipos de `src/types.ts`** sin autorización: son la fuente de verdad del contrato (ver [DATA_CONTRACT.md](../architecture/DATA_CONTRACT.md)).
