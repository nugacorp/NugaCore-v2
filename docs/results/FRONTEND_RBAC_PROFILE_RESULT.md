...
El frontend resolvía el rol consultando Supabase directo (anon key), pero **RLS deny-by-default** lo bloqueaba → todos caían a `Solo lectura` → todos veían lo mismo. Ahora el rol/perfil sale de **`/api/auth/me`** (backend, service-role). Detalle: [FRONTEND_AUTH_RBAC_AUDIT.md](../audits/FRONTEND_AUTH_RBAC_AUDIT.md).
